...
void getSample() {
  static long peakTime;
    ...
    #ifdef ESP32
      micIn = micData;
      if (digitalMic == false) micIn = micIn >> 2; 
    #endif
    #ifdef ESP8266
...
//                                                Range      |  Freq | Max magnitude on MAX9814 @ 40db gain (seems incorrect)
      ...
      fftCalc[3] = (fftAdd(7,9)) /3;        // 140 - 200   -> 170,   72000
      fftCalc[4] = (fftAdd(9,12)) /4;       // 180 - 260   -> 220,   60000
      fftCalc[5] = (fftAdd(12,16)) /5;      // 240 - 340   -> 290,   48000
      fftCalc[6] = (fftAdd(16,21)) /6;      // 320 - 440   -> 400,   41000
      fftCalc[7] = (fftAdd(21,28)) /8;      // 420 - 600   -> 500,   30000
      fftCalc[8] = (fftAdd(29,37)) /10;     // 580 - 760   -> 580,   25000
      fftCalc[9] = (fftAdd(37,48)) /12;     // 740 - 980   -> 820,   22000
      fftCalc[10] = (fftAdd(48,64)) /17;    // 960 - 1300  -> 1150,  16000
      fftCalc[11] = (fftAdd(64,84)) /21;    // 1280 - 1700 -> 1400,  14000
      fftCalc[12] = (fftAdd(84,111)) /28;   // 1680 - 2240 -> 1800,  10000
      fftCalc[13] = (fftAdd(111,147)) /37;  // 2220 - 2960 -> 2500,  8000
      fftCalc[14] = (fftAdd(147,194)) /48;  // 2940 - 3900 -> 3500,  7000
      fftCalc[15] = (fftAdd(194, 255)) /62; // 3880 - 5120 -> 4500,  5000

      for(int i=0; i< 16; i++) {
        fftCalc[i] = fftCalc[i] - noise[i];
...
void logAudio() {
  ...
  //------------ Oscilloscope output ---------------------------
    Serial.print("target Agc: "); Serial.print(targetAgc); Serial.println(" ");
    Serial.print("mult Agc: "); Serial.print(multAgc); Serial.println(" ");
    Serial.print("sample Agc: "); Serial.print(sampleAgc); Serial.println(" ");

    Serial.print("sample: "); Serial.print(sample); Serial.println(" ");
    Serial.print("sample avg: "); Serial.print(sampleAvg); Serial.println(" ");
    Serial.print("mic Lev: "); Serial.print(micLev); Serial.println(" ");
    Serial.print("sample Peak: "); Serial.print(samplePeak); Serial.println(" ");    //samplePeak = 0;
    Serial.print("mic In: "); Serial.print(micIn); Serial.print(" ");
